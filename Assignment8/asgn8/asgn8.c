...
int checkInMemory(unsigned int value){
	...
	for(i=(physMem->front); i<(physMem->rear); i++){
		puts("entered");
...
/*
 * function to shift all elements of queue by 1
 */
void shiftElements(struct Queue * q){
	int i;
	for(i=(q->capacity-1); i==1; i--){
		q->array[i]->val = q->array[i-1]->val;
		q->array[i]->empty = q->array[i-1]->empty;
	}
...
void fifo(unsigned int value){
	...
	if(test1 == 0){ //page is not loaded in memory
		...
		if(!isFull(physMem)){
			...
			return;
		}else{ //physical memory is full and a page needs to be removed (page fault)
			...
			if(test2==1){ //if more frames are needed
				...
				shiftElements(physMem); //shift all elements of queue by 1 to keep FIFO
				enqueue(physMem, newPage); //insert new page
				...
				physMem->array = realloc(physMem->array, numberFrames); //reallocate memory for array
				...
				if(enqueue(physMem, newPage)==-1){
					dequeue(physMem);
					enqueue(physMem, newPage);
					printf("Normal, capacity = %d, size = %d, frames = %d, frequency = %f\n", physMem->capacity, physMem->size, numberFrames, frequency);
				}
			}
		}
...
int main(int argc, char ** argv){
	...
	while(scanf("%d ", &value)!=EOF){
		...
		printf("array front is at pos %d, array rear is at pos %d\n", physMem->front, physMem->rear);
		if(algorithm == 0){
			fifo(value);
		}else if(algorithm == 1){
			lru(value);
