...
//		strcat(line, "\n");
	...
	int current_time = arrival[0];//each cpu gets its own timer
	//memset(current_time[0], arrival[0], sizeof(currnet_time));//set all cpu's timers to the arrival of first element
	char cpu[proc];
	while(all<=count){
		THERE: if(alldone==1){
			printf("\n%d\tIDLE", ++current_time);
		...
		THIS2:for(k=0;k<proc; k++){
			...
			cpu[line_lim] = process[l];
			FLAG1:for(u=0; u<duration[l]; u++){
				if(remaining[l]!=0){
				printf("%c",cpu[line_lim]);
				line_lim--;
				if(line_lim==0){
					printf("\n");
					current_time++;
				...
				for(pr=0; pr<count; pr++){
					...
					if((pr!=l)&&(deadline[pr]==deadline[l])&&(remaining[pr]<remaining[k])&&(remaining[pr]!=0)&&(arrival[pr]<=current_time)){//if there is a process with a shorter duration
						l=pr;
						cpu[line_lim] = process[l];
						goto FLAG1;
					}//end if
